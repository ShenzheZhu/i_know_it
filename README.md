...
- Region monitoring is experimental. A real macOS Control + 4 test reproduced missing region metadata on version 0.5.2: the image and context file were preserved, but the companion expected the default shortcut. Version 0.5.3 reads the configured shortcut; live region metadata acceptance remains pending. The panel's monitoring status indicates that an event tap was established, not that a screenshot was successfully attributed.
