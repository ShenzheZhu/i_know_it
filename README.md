...
4. For observed region coordinates with **Control + Shift + Command + 4**, click **Allow region context** in the switch panel, then allow **Input Monitoring** in macOS settings. This is a one-time setup action, separate from taking screenshots. macOS determines whether permission is attributed to Chrome or its native companion; use the entry shown by the system. Reopen Chrome if macOS asks you to restart it.
...
With Input Monitoring allowed, the companion passively observes the existing **Control + Shift + Command + 4** shortcut and a plain region drag. It does not replace the shortcut, post input, take its own screenshots, or store keyboard text/history. A single new clipboard image within two seconds of mouse-up must have exactly the dimensions predicted by that display's backing scale. The context then includes the observed selection in global display points, the display's bounds and scale, and selection pixels relative to that display. Negative secondary-display origins are retained. Full-display coordinates therefore do not depend on stretching the cropped image to a standard aspect ratio.
...
- Region monitoring is experimental: automated checks cover decoded input and private pasteboards, not delivery of a real macOS screenshot shortcut. Input Monitoring permission and real shortcut-to-Codex paste acceptance remain pending. The panel's monitoring status indicates that an event tap was established, not that a screenshot was successfully attributed.
