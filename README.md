...
This is an experimental macOS implementation. Chrome transport and clipboard behavior are tested. One real macOS region capture has also been verified through image and Markdown attachments supplied by the user in Codex desktop. Other agent platforms, including Claude Code terminals, are not yet supported.
...
4. For observed region coordinates with your macOS **Copy picture of selected area to the clipboard** shortcut, click **Allow region context** in the switch panel, then allow **Input Monitoring** in macOS settings. The default shortcut is **Control + Shift + Command + 4**; customized bindings are read from System Settings. This is a one-time setup action, separate from taking screenshots. macOS determines whether permission is attributed to Chrome or its native companion; use the entry shown by the system. When you return to Chrome after opening the permission settings, the extension reconnects its companion once to refresh cached permission state. Follow any restart instruction from macOS if the panel still reports unavailable.
...
The installer saves a local build receipt. Running it again reuses the installed executable, preserving its bytes and modification time, when the native source, compiler identity, architecture, and executable hash still match the receipt. This avoids an unnecessary rebuild for extension-only updates. An older installation without a receipt is rebuilt once; changes to those inputs also require a rebuild. macOS may require Input Monitoring permission again after a rebuild. The receipt does not grant or repair system permissions.
...
On readable HTTP(S) pages, an experimental estimate can also express the raw drag in viewport and top-document CSS pixels. It uses one recent, trusted mouse screen/client observation together with browser zoom to estimate the content viewport's screen origin. The page retains only that observation in memory, not a pointer history; OFF clears it. An estimate requires agreement between page and browser window measurements, display scale, and unchanged geometry, with the whole viewport on the captured display and the drag inside it. Missing or stale observations, pinch zoom, full-screen mode, and ambiguous geometry leave CSS coordinates unknown. This happens passively and adds no screenshot step. On macOS Chrome, nine pointer/DOM samples paired with independent native accessibility bounds and seven additional edge samples passed conversion checks within one CSS pixel, across 80%, 90%, 100%, 110%, 125%, and 150% zoom and a scrolled page. This reproduced and fixed Float32 pointer rounding at display boundaries; only 0.001 display point of edge tolerance is allowed. The full system-screenshot-to-CSS-metadata handoff still needs acceptance.
...
- macOS spaces, physical monitor switching, third-party screenshot tools, and broader native Codex composer flows remain unverified beyond the single received image/context pair described below. Windows, Linux, remote agents, and Claude Code are not supported by this companion.
