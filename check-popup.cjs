...
const chrome = {
  runtime: { id: 'fixture', getURL: file => `chrome-extension://fixture/${file}`, onInstalled: event(), onStartup: event(), onMessage: event(),
    ...
    connectNative() { connection = { postMessage: message => native.push(message), onMessage: event(), onDisconnect: event() }; return connection; } },
...
function popupChrome() {
  globalThis.chrome = { runtime: { id: 'fixture', sendMessage: message => window.fixtureMessage(message),
...
(async () => {
  ...
  try {
    ...
    assert.equal(native.filter(m => m.type === 'request-input-access').length, 1, 'Toggles, reloads and reconnect errors must never request permission');
    await nativeStatus(page, 'ready');
      ...
      for (const colorScheme of ['light', 'dark']) {
        ...
        for (const initialWidth of [25, 181, 280, 320]) {
          ...
          const bounds = await popup.evaluate(() => {
            ...
            return { viewport: innerWidth, height: innerHeight, body: rect('body'), card: rect('.control'), track: rect('.track'), input: rect('#enabled'), text: rect('#switch-title'), permission: rect('#allow-region'), overflow: document.documentElement.scrollWidth > innerWidth };
          });
    ...
    const report = { checkedAt: new Date().toISOString(), checks: ['Popup opens without changing state or requesting permission', 'Mouse and keyboard switch update background, native host messages and badge', 'Reopening restores state', 'Pending switch and permission requests disable duplicate input', 'Only explicit permission button click sends a request; toggles and reloads do not', 'Native status updates the open popup without polling; forged status messages are rejected', 'OFF hides region setup, host disconnect clears ready status', 'Lost reply reads back actual state', 'Disconnected states stay disabled and recover on reopen', 'Light/dark layout, reduced motion and no script errors', 'Popup preferred-size feedback at 25/181/280/320 px, DPR 1/2, card and permission button containment and aligned click target'], layouts, scope: 'Real popup DOM and background code with fixture Chrome APIs and simulated popup sizing feedback; no extension-manager or Codex UI control' };
